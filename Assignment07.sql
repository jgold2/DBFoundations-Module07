...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JaquelynGoldsberry')
	 Begin 
	  Alter Database [Assignment07DB_JaquelynGoldsberry] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JaquelynGoldsberry;
	 End
	Create Database Assignment07DB_JaquelynGoldsberry;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JaquelynGoldsberry;
...
-- Format unit price in US dollars
Select
	ProductName,
	[Price] = '$' + Cast(UnitPrice as nvarchar(50))
From vProducts
*/
...
-- Join tables on category ID and format price as US dollars
Select 
	CategoryName, 
	ProductName,
	[Price] = '$' + Cast(UnitPrice as nvarchar(50))
...
-- Join on product ID and select relevant columns
Select
	ProductName,
...
-- Join through product ID and category ID & select columns
Select 
	CategoryName,
	InventoryDate,
	i.Count
...
Select 
	...
	[PreviousMonthCount] = IIF(
		Month(InventoryDate) = 1,0,  -- If January set previous month to 0
		Lag(i.Count) Over (Order By Productname, Month(InventoryDate))
	)
From dbo.vProducts as p
	Join dbo.vInventories as i on p.ProductID = i.ProductID
...
-- <Put Your Code Here> --
/*
-- Start with results from Q6 Create KPI
 Select 
	ProductName,
	InventoryDate,
	InventoryCount,
